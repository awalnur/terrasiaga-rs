...
impl Default for SecurityConfig {
    fn default() -> Self {
        ...
        rate_limits.insert("auth_login".to_string(), RateLimitConfig {
            max_requests: 5,
            window_seconds: 300, // 5 minutes
            burst_allowance: 2,
        });
        
        rate_limits.insert("disaster_report".to_string(), RateLimitConfig {
            max_requests: 10,
            window_seconds: 3600, // 1 hour
            burst_allowance: 3,
        });
        
        rate_limits.insert("general_api".to_string(), RateLimitConfig {
            max_requests: 100,
...
impl ProductionSecurityService {
    ...
    pub async fn validate_jwt_token(&self, token: &str) -> AppResult<AuthSession> {
        ...
        let session: AuthSession = self.cache.get(&session_key).await?
    ...
    pub async fn check_rate_limit(&self, user_id: &str, action: &str, ip_address: Option<&str>) -> AppResult<bool> {
        let config = self.config.rate_limits.get(action)
            .unwrap_or(&self.config.rate_limits["general_api"]);
        ...
        let rate_key = if !user_id.is_empty() {
            ...
            format!("rate_limit:ip:{}:{}", ip, action)
        } else {
            format!("rate_limit:anonymous:{}", action)
        };

        // Get current request count
        let current_count = self.cache.get::<u32>(&rate_key).await?.unwrap_or(0);
        ...
        if new_count == 1 {
            self.cache.expire(&rate_key, Duration::from_secs(config.window_seconds)).await?;
        }
    ...
    pub async fn track_failed_login(&self, email: &str, ip_address: Option<&str>) -> AppResult<bool> {
        ...
        if attempts == 1 {
            // Set expiration for failed attempts tracking
        ...
        if attempts as u32 >= self.config.max_failed_attempts {
            ...
            self.cache.set(&lockout_key, &true, Some(Duration::from_secs(self.config.lockout_duration_minutes * 60))).await?;
            
            // Also track IP-based lockout if available
            if let Some(ip) = ip_address {
                let ip_lockout_key = format!("lockout:ip:{}", ip);
                self.cache.set(&ip_lockout_key, &true, Some(Duration::from_secs(self.config.lockout_duration_minutes * 60))).await?;
            }
    ...
    pub async fn update_session_activity(&self, session_id: &str) -> AppResult<()> {
        ...
        if let Ok(Some(mut session)) = self.cache.get::<AuthSession>(&session_key).await {
...
impl AuthService for ProductionSecurityService {
    ...
    async fn revoke_token(&self, token: &str) -> AppResult<()> {
        ...
        let token_data = decode::<JwtClaims>(token, &self.decoding_key, &Validation::new(Algorithm::HS256))
            .map_err(|e| AppError::Unauthorized(format!("Invalid token: {}", e)))?;
